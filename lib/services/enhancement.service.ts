...
import { sogniTextComplete } from "@/lib/providers/sogni/sogni.text";
import { pollinationsTextComplete } from "@/lib/providers/pollinations/pollinations.text";
...
/**
 * AI engines, tried in order before the deterministic fallback. Sogni rides
 * the already-configured render account; Pollinations is the keyless
 * second chance; both are screened by the sanitizer, so a budget apology or
 * empty reasoning reply simply falls through to the next engine.
 */
const ENGINES: { id: string; complete: typeof sogniTextComplete }[] = [
  { id: "sogni", complete: sogniTextComplete },
  { id: "pollinations", complete: pollinationsTextComplete },
];
